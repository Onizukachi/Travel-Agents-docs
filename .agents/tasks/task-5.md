...
- Минимальный курс пересчета трактовать как `max(курс на момент бронирования/замены пакета, курс последней оплаты туриста, внутренний курс с маркапом)`, но формулу маркапа нужно подтвердить.
  Обоснование: в ТЗ есть нижняя граница и рекомендация учитывать маркап; пользователь подтвердил курс бронирования как `package.stat_exchange_rate`, а источник маркапа как `markup_applications`/`markup_rules` с типом `exchange_rate`, но способ перевода этого маркапа в минимально допустимый курс еще нужно уточнить.
...
- Уже подтверждены пользователем бизнес-решения:
  ...
  - одновременно допускается только один активный пересчет на заказ;
...
- Открытые вопросы:
  - как именно вычислять минимальный курс на основе валютного маркапа: брать готовый applied markup из пакета, пересчитывать курс из рублевой дельты или искать другой источник;
  - что делать после автоотмены в 18:00: разрешать новый пересчет в тот же день или только на следующий;
  - нужно ли логировать отклоненные preview-попытки в отдельную историю или достаточно хранить только примененные/отмененные записи.
